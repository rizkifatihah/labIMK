...
class Users extends CI_Controller {
	...
	public function create($param1='',$param2=''){
		...
		if ($param1=='do_create') {
			...
			$session_username = $this->session->userdata('nim');
			$exists = $this->UsersModel->getNIMByid($nim);
			$count = count($exists);
			if($count > 0 AND $username != $session_username ){
				$this->session->set_flashdata('notif','<div class="alert alert-danger">NIM sudah digunakan</div>');
