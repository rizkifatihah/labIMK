...
class Auth extends CI_Controller {
  ...
  public function login($param1='',$param2=''){
		if ($param1=='do_login') {
			...
			$auth = $this->AuthModel->getAccountLogin($nim,$password);
			if ($auth) {
				foreach ($auth as $key) {
					$loginSession = array(
				...
				$onlineStatus = $this->GeneralModel->update_general('tb_pengguna','nim',$nim,$activity_status);
				redirect('panel/dashboard');
			}else {
				$this->session->set_flashdata('notif','<div class="alert alert-danger">Nim atau Password yang anda masukkan salah!</div>');
				redirect('/auth/login');
			}
		}else {
			$this->load->view('login');
